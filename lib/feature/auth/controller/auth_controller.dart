...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> login(
    ...
    if (response!.statusCode == 200 &&
      ...
      await _saveTokenAndNavigate(
          ...
          password: password, toPage: 'login');
  ...
  _saveTokenAndNavigate(
    ...
    } else if(toPage == "cartScreen"){
      print("print krwa rha hu $toPage");
        Get.toNamed(RouteHelper.getCartRoute());
    }else {
      print(" this is login home $toPage");
      print("print karaya he $toPage");
    ...
    if (response!.statusCode == 200 &&
      ...
      if (fromPage == "profile") {
        ...
        await _saveTokenAndNavigate(
            ...
            token: response.body['content']['token'],
            emailPhone: "",
            password: "", toPage: 'verifyOtpForVerificationScreen');
      }
    } else {
    ...
    if (response!.statusCode == 200) {
      if (response.body['content']['token'] != null) {
        await _saveTokenAndNavigate(
            ...
            token: response.body['content']['token'],
            emailPhone: phone,
            password: "", toPage: toPage);
        print("hnn yhi he jo he ${RouteHelper.getMainRoute("home")} $toPage");
    ...
    if (response!.statusCode == 200) {
      if (response.body['content']['token'] != null) {
        await _saveTokenAndNavigate(
            fromPage: RouteHelper.getMainRoute("home"),
            token: response.body['content']['token'],
            emailPhone: phone, toPage: 'verifyOtpForFirebaseOtp');
      } else if (response.body['content']['temporary_token'] != null) {
        Get.offNamed(RouteHelper.getUpdateProfileRoute(phone: phone ?? ""));
      }
    } else {
    ...
    if (response.statusCode == 200) {
      if (response.body['content']['token'] != null) {
        await _saveTokenAndNavigate(
            ...
            token: response.body['content']['token'],
            emailPhone: phone,
            password: "", toPage: 'updateNewUserProfileAndLogin');
      }
    } else {
    ...
    if (response.statusCode == 200) {
      if (response.body['content']['token'] != null) {
        await _saveTokenAndNavigate(
            ...
            token: response.body['content']['token'],
            emailPhone: "",
            password: "", toPage: 'registerWithSocialMedia');
      } else if (response.body['content']['temporary_token'] != null) {
        var config = Get.find<SplashController>().configModel.content;
  ...
  Future<void> existingAccountCheck(
    ...
    if (response.statusCode == 200) {
      if (response.body['content']['token'] != null) {
        await _saveTokenAndNavigate(
            fromPage: RouteHelper.getMainRoute("home"),
            token: response.body['content']['token'], toPage: 'existingAccountCheck');
      } else if (response.body['content']['temporary_token'] != null) {
